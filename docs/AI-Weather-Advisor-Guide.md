...
AI Weather Advisor 根據當前顯示模式自動切換 Prompt，提供不同情境下的建議。

#### 一般模式 Prompt (Light/Dark Mode)
...
判斷準則 (優先級由高至低)：
1. 變天預警（最高優先）→ 如果現在晴朗但未來會下雨，提醒帶傘
2. 溫差預警 → 如果溫度變化超過 3-5°C，提醒穿搭
3. 危險/極端天氣 → 優先警告安全
4. 降雨 (>60%) → 提醒帶傘或行車安全
5. 極端溫度 (體感 <12°C 或 >32°C) → 提醒保暖或防曬/補水
6. 舒適/一般 → 給予祝賀或心情小語
```

#### 睡眠模式 Prompt (Sleep Mode) 🌙

當進入睡眠模式 (23:00-06:00) 時，AI 會切換為「夜間管家」角色：
...
任務目標：
根據當前與半夜的氣溫、濕度，給予使用者「睡眠環境」的建議，並道晚安。
...
1. **進入睡眠模式時：**
   - 檢查 `lastSleepMessageDate` 是否等於今天
   - 若不同（今晚還沒說晚安）→ 呼叫 LLM 生成晚安訊息
   - 若相同（已經說過晚安）→ **跳過** API 呼叫，保持原訊息
   - 例外：手動切換到 Sleep Mode 時，強制更新

2. **睡眠模式期間：**
   - 暫停常規的天氣建議更新
   - 不呼叫 LLM API，直到離開睡眠模式
...
**節省成本：**
- 睡眠期間（23:00-06:00）= 7 小時 = 0 次 API 呼叫
- 傳統方案（每 30 分鐘）= 14 次 API 呼叫
- **節省：** 每晚節省 13-14 次 API 呼叫 (~NT$0.03)
- **月省：** 約 400 次 API 呼叫 (~NT$0.9)
...
### v1.2.0 (2026-01-09)
- ✨ 新增睡眠模式 (Sleep Mode) with 專屬 AI 晚安訊息
- ✨ 實作睡眠模式 API 呼叫優化（每晚只說一次晚安）
- ✨ 新增睡眠模式專屬 Prompt 和 Fallback 訊息
- 🔧 簡化顯示模式管理（從 4 個狀態變數減少到 1 個）
- 📝 更新文檔說明睡眠模式功能
